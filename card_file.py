...
class card:
    ...
    def create_card(name:str):
        cards = pd.read_csv(f"{BASE_PATH}/card.txt",index_col="name")
        return card(name,cards.loc[name].iloc[1:27].to_numpy(dtype=bool),cards.loc[name]["Stamp"] == "Blue")
    
    
    def create_5_random_cards():
        cards = pd.read_csv(f"{BASE_PATH}/card.txt")
        selections = cards.sample(n=5)
        return np.array(
            [card(selection.iloc[0],
            selection.iloc[2:27].to_numpy(dtype=bool),
            selection.iloc[1] == "Blue") for i,selection in selections.iterrows()]
            )
